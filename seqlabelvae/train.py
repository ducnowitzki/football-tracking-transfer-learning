...
def build_architecture():
    ...
    MLP_a = model_utils.get_timewise_MLP_a(timesteps, intermediate_dim, hidden_dim)
...
def get_model(pitch_x_axis=105, pitch_y_axis=68, channels=3, feature_dim=300, intermediate_dim=128, hidden_dim=16, timesteps=20,\
    ...
    # f: (timesteps, intermediate_dim + hidden_dim) -> (timesteps, hidden_dim)
    MLP_z = model_utils.get_timewise_MLP_z_1(timesteps, intermediate_dim, hidden_dim)
...
def compute_loss(model, x_labeled, y, x_unlabeled, labeled_batch_size, unlabeled_batch_size, \
    ...
    kl_loss = kl_weight*(tf.maximum(kl_loss_z, kl_min) + tf.maximum(kl_loss_a, kl_min)) 
...
def train():
    ...
    # data
    labels_path = opt.labels
    labeled_sequences_path = opt.labeled_sequences

    unlabeled_frames_path = opt.unlabeled_frames

    #resolve paths
    labels_path = os.path.abspath(labels_path)
    labeled_sequences_path = os.path.abspath(labeled_sequences_path)
    unlabeled_frames_path = os.path.abspath(unlabeled_frames_path)
    

    # training dataset
    y = np.load(labels_path, mmap_mode='r')

    labeled_dataset = np.load(labeled_sequences_path, mmap_mode='r')
    unlabeled_dataset = np.load(unlabeled_frames_path, mmap_mode='r')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_path', type=str, required=True, help='Location to save weights')
