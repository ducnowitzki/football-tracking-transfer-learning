...
parser.add_argument('--pitch_x_axis', type=int, default=31)
parser.add_argument('--pitch_y_axis', type=int, default=20)
...
focal_loss_fn = tf.keras.losses.CategoricalFocalCrossentropy(
    ...
    alpha=[0.02, 0.40, 0.58],  # order = [pass, shot, dribble]
    from_logits=False
)
...
def train():
    ...
    # Calculate total possible sequences from unlabeled data
    total_possible_sequences = unlabeled_frames.shape[0] - args.timesteps + 1
    sequences_per_epoch = int(args.sample_rate * total_possible_sequences)
    ...
    for epoch in range(args.epochs):
        ...
        unlabeled_indices = np.random.permutation(total_possible_sequences)
        ...
        for batch in range(n_batches):
            ...
            x_unlabeled = np.array([
                ...
                for i in current_unlabeled_indices
            ])
