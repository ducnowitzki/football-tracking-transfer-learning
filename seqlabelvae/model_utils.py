...
# own conttribution
def get_timewise_MLP_z_1(timesteps, input_dim, hidden_dim):
    """
    Builds a time-distributed MLP for latent variable z
    Input: shape (timesteps, input_dim)
    Output: z_mean_t, z_log_sigma_t, z_t
    """
    inp = layers.Input((timesteps, input_dim))
    z_mean_t = layers.TimeDistributed(layers.Dense(hidden_dim))(inp)
    z_log_sigma_t = layers.TimeDistributed(layers.Dense(hidden_dim))(inp)
    # Sampling_timestep() as in your framework
    z_t = Sampling_timestep()([z_mean_t, z_log_sigma_t])
    model = keras.Model(inp, [z_mean_t, z_log_sigma_t, z_t])
    return model
...
def get_DCGAN_reverse_feature_extractor(feature_dim, image_0, image_1, channels):
    ...
    x = layers.Dense(units=int(128*int((1/4)*image_0)*(1/4)*image_1))(z_inp)
